...
char VigenereDecrypt::operator()(const char &c)
{
    char r {};
    int shift {};
    ...
    if (isalpha(c))
    {
        ...
        if (vigenere_table[toupper(c)]-shift >= 0 )
            r=alphabet[vigenere_table[toupper(c)]-shift];
        else
            r=alphabet[vigenere_table[toupper(c)]-shift+26];
